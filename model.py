...
class Gen_Model():
    ...
    def printWeightAverages(self):
        ...
        for i, l in enumerate(layers):
            try:
                x = l.get_weights()[0]
                lg.logger_model.info('WEIGHT LAYER %d: ABSAV = %f, SD =%f, ABSMAX =%f, ABSMIN =%f', i, np.mean(
                    np.abs(x)), np.std(x), np.max(np.abs(x)), np.min(np.abs(x)))
            except:
                pass
        ...
        for i, l in enumerate(layers):
            try:
                x = l.get_weights()[1]
                lg.logger_model.info('BIAS LAYER %d: ABSAV = %f, SD =%f, ABSMAX =%f, ABSMIN =%f', i, np.mean(
                    np.abs(x)), np.std(x), np.max(np.abs(x)), np.min(np.abs(x)))
            except:
                pass
        lg.logger_model.info('******************')
        ...
        for i, l in enumerate(layers):
            ...
            try:
                ...
                for i in range(s[2] * s[3]):
                    ...
                    sub.imshow(weights[:, :, channel, filter],
                               cmap='coolwarm', clim=(-1, 1), aspect="auto")
...
class Residual_CNN(Gen_Model):
    def __init__(self, reg_const, learning_rate, input_dim, output_dim, hidden_layers):
        Gen_Model.__init__(self, reg_const, learning_rate,
    ...
    def residual_layer(self, input_block, filters, kernel_size):
        ...
        x = Conv2D(
            ...
            activation='linear', kernel_regularizer=regularizers.l2(self.reg_const)
        )(x)
    ...
    def value_head(self, x):
        ...
        x = Dense(
            20, use_bias=False, activation='linear', kernel_regularizer=regularizers.l2(self.reg_const)
        )(x)

        x = LeakyReLU()(x)

        x = Dense(
            1, use_bias=False, activation='tanh', kernel_regularizer=regularizers.l2(self.reg_const), name='value_head'
        )(x)
    ...
    def policy_head(self, x):
        ...
        x = Dense(
            self.output_dim, use_bias=False, activation='linear', kernel_regularizer=regularizers.l2(self.reg_const),
            name='policy_head'
        )(x)
